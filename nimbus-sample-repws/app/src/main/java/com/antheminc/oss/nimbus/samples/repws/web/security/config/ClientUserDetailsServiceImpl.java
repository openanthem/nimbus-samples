package com.antheminc.oss.nimbus.samples.repws.web.security.config;
...
import com.antheminc.oss.nimbus.samples.repws.ApplicationBeanResolver;
...
public class ClientUserDetailsServiceImpl implements ClientUserDetailsService {
	...
	public static final Logger LOG = LoggerFactory.getLogger(ClientUserDetailsServiceImpl.class);

	private final SessionProvider sessionProvider;

	@Override
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		String searchUri = "/anthem/ltss/p/clientuser/_search?fn=query&where=clientuser.loginId.eq('" + username
				+ "')&fetch=1";
	...
	private void createResolvedAccessEntities(ClientUser clientUser) {
		...
		Set<String> userRoleCodes = clientUser.getRoles().stream().map(UserRole::getRoleCode)
		...
		String searchAuthoritiesUri = "Anthem/fep/ltss/p/authorities/_search?fn=query&where=authorities.code.in("
				+ sb2.toString() + ")";
	...
	public ClientUserDetails getClientUserDetails() {
		ClientUser petclinic_user = sessionProvider.getLoggedInUser();
