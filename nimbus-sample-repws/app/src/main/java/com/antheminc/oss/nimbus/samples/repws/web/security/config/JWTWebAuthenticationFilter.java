...
import com.antheminc.oss.nimbus.samples.repws.web.security.utils.JWTAuthConstants;
import com.antheminc.oss.nimbus.samples.repws.web.security.utils.JWTAuthTokenUtil;
...
public class JWTWebAuthenticationFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
			DefaultClientUserDetails userDetails = (DefaultClientUserDetails) userDetailsService
					.loadUserByUsername(username);
			if (tokenUtil.validateToken(authToken, userDetails)) {
				logit.trace(() -> "@@ webauthtoken valid for logged in user " + userDetails.getUsername()
